...
struct Sink {
  ...
  bool select(uint8_t source) {
    if (source >= sources_number(SinkId)) {
      Debug::log("{} is outside the range of valid sources, [0-{}), of pin {}.", source, sources_number(SinkId),
                 SinkId);
      return false;
    }
    *reg = 1 << source;
...
struct PinSinks : private utils::NoCopyNoMove {
  ...
  Sink<PinSink> get(PinSink sink) volatile { return get_sink<PinSink>(registers, sink); };
};
...
struct BlockSinks : private utils::NoCopyNoMove {
  ...
  Sink<BlockSink> get(BlockSink sink) volatile { return get_sink<BlockSink>(registers, sink); };
};
}  // namespace SonataPinmux
