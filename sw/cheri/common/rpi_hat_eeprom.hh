...
// header).
bool parse_header(const uint8_t buf[], uint32_t buf_size, eeprom_header_t& header_out) {
  if (buf_size < 12) {
    return false;
  }
...
// correct.
bool parse_atom_info(const uint8_t buf[], uint32_t start_index, uint32_t buf_size, atom_info_t& info_out) {
  ...
  uint16_t calc_crc = calc_crc16(buf, start_index + AtomHeaderLength, AtomHeaderLength + info_out.length - 2);
...
bool parse_vendor_info(const uint8_t buf[], uint32_t start_index, uint32_t buf_size, vendor_atom_t& vendor_atom_out) {
