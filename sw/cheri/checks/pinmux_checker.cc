...
static bool i2c_read_bh1745_id_test(I2cPtr i2c) {
  ...
  i2c->blocking_write(Bh1745DeviceAddr, addr, sizeof(addr), false);
 ...
 * Returns true if the test passed, or false if it failed.
 */
static bool spi_n25q256a_read_jedec_id(Capability<volatile SonataSpi> spi, Capability<volatile SonataGpioFull> gpio,
                                       GpioPin spi_cs_pin) {
 ...
 */
static bool execute_i2c_test(const Test &test, I2cPtr i2cs[2]) {
 ...
 * the test.
 */
static bool execute_spi_test(const Test &test, Capability<volatile SonataGpioFull> gpio,
                             Capability<volatile SonataSpi> spis[2]) {
  Capability<volatile SonataSpi> tested_spi = spis[static_cast<uint8_t>(test.spi_data.spi) - 3];
 ...
 * otherwise.
 */
bool execute_test(const Test &test, uint32_t test_num, ds::xoroshiro::P32R8 &prng,
  ...
  if (test.manual_required) {
    write_str(console, prefix);
    write_str(console, test.instruction);
    write_str(console, "\r\n");
    write_str(console, prefix);
    write_str(console, "\x1b[36mPress the joystick when you are ready to continue.\033[0m");
  ...
  write_str(console, prefix);
  write_str(console, "  Executing test ");
  write_hex8b(console, test_num);
  write_str(console, ": ");
  write_str(console, test.name);
  write_str(console, " ... ");
  ...
  for (uint32_t i = 0; i < test.num_output_pins; i++) {
    ...
    if (!pinmux->output_pin_select(pin_assign.pin, pin_assign.select)) {
      write_str(console, "\r\n");
      write_str(console, prefix);
      write_str(console, "Error: invalid pinmux output pin selection.");
      return false;
    }
  }
  for (uint32_t i = 0; i < test.num_block_inputs; i++) {
    ...
    if (!pinmux->block_input_select(block_assign.input, block_assign.select)) {
      write_str(console, "\r\n");
      write_str(console, prefix);
      write_str(console, "Error: invalid pinmux block input selection.");
      return false;
    }
  }
  ...
  switch (test.type) {
    ...
    case TestType::I2cBH1745ReadId:
      passed = execute_i2c_test(test, i2cs);
      break;
    ...
    default:
      write_str(console, "\r\n");
      write_str(console, prefix);
      write_str(console, "Error: unknown test type.");
  }
  write_test_result(console, (passed ? 0 : 1));
  return passed;
}
 ...
 */
bool ask_retry_last_test(UartPtr console, Capability<volatile SonataGpioFull> gpio) {
  write_str(console, prefix);
  write_str(console, "\x1b[36mPrevious test failed. Move the joystick to retry, or press it to continue.\033[0m");
  ...
  if (!was_pressed) {
    write_str(console, prefix);
    write_str(console, "\x1b[36mRepeating previous test.\033[0m\r\n");
    return true;
  }
  return false;
}
...
bool execute_testplan(Test *testplan, uint8_t NumTests, ds::xoroshiro::P32R8 &prng,
  ...
  write_str(console, "\r\n");
  write_str(console, prefix);
  write_str(console, "Starting check.\r\n\r\n");
  ...
  while (current_test < NumTests) {
    ...
    if (PinmuxEnableRetry && !last_test_passed && ask_retry_last_test(console, gpio)) {
      current_test--;
      retries++;
    }
    ...
    current_test++;
    if (!execute_test(test, current_test, prng, gpio, console, uarts, spis, i2cs, pinmux)) {
  ...
  write_str(console, "\r\n");
  write_str(console, prefix);
  write_str(console, "Check result: ");
  write_hex8b(console, passes);
  write_str(console, "/");
  write_hex8b(console, NumTests);
  write_str(console, " tests pass");
  if (retries > 0) {
    write_str(console, " (");
    write_hex8b(console, retries);
    write_str(console, " retries)");
  }
  write_str(console, " ....... ");
  write_test_result(console, static_cast<uint8_t>(passes != current_test));
