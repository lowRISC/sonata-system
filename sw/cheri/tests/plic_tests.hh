...
struct PlicTest {
  ...
  void gpio_test() {
    ...
    raw_gpio_ptr[4]   = 0x01;  // PCINT mask: pin 0 only
    raw_gpio_ptr[5]   = 0x01;  // ctrl & stats: clear status, any-edge, non-debounced, intr disabled
    ...
    irq_handler      = [](PlicTest *plic_test, PLIC::Interrupts irq) {
      ...
      plic_test->error_count += wrong_local;
      // TODO: use names rather than cast+offset once gpio_ptr includes PCINT regs
      plic_test->log(irq, (raw_gpio_ptr[5] >> 16) & raw_gpio_ptr[5], wrong_irq || wrong_local);
