 ...
 */
static int pinmux_gpio_test(SonataPinmux *pinmux, Capability<volatile SonataGpioFull> gpio, UartPtr console) {
...
static int pinmux_mux_test(SonataPinmux *pinmux, ds::xoroshiro::P32R8 &prng, UartPtr uart3,
  ...
  constexpr uint8_t PmxArduinoD1ToUartTx3     = 1;
 ...
 */
void pinmux_tests(CapRoot root, UartPtr console) {
  ...
  for (size_t i = 0; i < PINMUX_TEST_ITERATIONS; i++) {
    ...
    write_str(console, "\\");
    write_hex8b(console, PINMUX_TEST_ITERATIONS - 1);
    write_str(console, "\r\n\x1b[35m(may need manual pin connections to pass)");
    write_str(console, "\x1b[0m\r\n");

    bool test_failed = false;
    int failures     = 0;

    write_str(console, "  Running SPI Flash Pinmux test... ");
    ...
    if (I2C_RPI_HAT_AVAILABLE) {
      write_str(console, "  Running I2C Pinmux test... ");
    ...
    if (PINMUX_CABLE_CONNECTIONS_AVAILABLE) {
      write_str(console, "  Running GPIO Pinmux test... ");
      failures = pinmux_gpio_test(&Pinmux, gpio_full, console);
      test_failed |= (failures > 0);
      write_test_result(console, failures);

      write_str(console, "  Running UART Pinmux test... ");
      failures = pinmux_uart_test(&Pinmux, prng, uart3);
      test_failed |= (failures > 0);
      write_test_result(console, failures);

      write_str(console, "  Running Mux test... ");
