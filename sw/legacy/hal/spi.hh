...
class Spi {
 ...
 public:
  ...
  void blocking_write(const uint8_t array[], std::size_t size) {
    if (size == 0) {
      return;
    }
  ...
  void blocking_read(uint8_t array[], std::size_t size) {
  ...
  template <std::size_t CS, bool DEASERT_OTHERS = true>
  inline void chip_select_assert(bool assert = true) {
    mmio::spi::Spi spi(base_addr);
    if constexpr (DEASERT_OTHERS) {
      spi.cs.fetch().cs.write(assert << CS).commit();
    } else {
      spi.cs.fetch().cs.bit_mask(assert, CS).commit();
    }
  }
};
