...
module gpio_core #(
  ...
  localparam int unsigned GPIO_PIN_MASK_REG = 32'h10;
  localparam int unsigned GPIO_CTRL_STS_REG = 32'h14;
  ...
  logic pcint_mask_sel, pcint_mask_rd_en;
  logic ctrl_sts_sel, ctrl_sts_rd_en;

  logic [DbncWidth-1:0] dbnc_cnt;
  logic dbnc_step;

  logic [GpoWidth-1:0] pcint_mask, pcint_mask_d;
  ...
  always @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      pcint_enable <= (ctrl_sts_sel && device_we_i)   ? pcint_enable_d : pcint_enable;
      pcint_mode   <= (ctrl_sts_sel && device_we_i)   ? pcint_mode_d   : pcint_mode;
      pcint_i_sel  <= (ctrl_sts_sel && device_we_i)   ? pcint_i_sel_d  : pcint_i_sel;

      // pcint_status is set by hardware and only cleared when a '1' is written
      if (pcint_triggered) begin
        pcint_status <= 1'b1;
      end else if (ctrl_sts_sel && device_we_i && pcint_status_d) begin
        pcint_status <= 1'b0;
      end else begin
      ...
      pcint_mask_rd_en  <= pcint_mask_sel  & ~device_we_i;
      ctrl_sts_rd_en    <= ctrl_sts_sel    & ~device_we_i;
    end
  end
  ...
  for (genvar i_byte = 0; i_byte < 4; ++i_byte) begin : g_gp_o_d;
    if (i_byte * 8 < GpoWidth) begin : gen_gp_o_d_inner
      ...
      assign pcint_mask_d[gpo_byte_end - 1 : i_byte * 8] =
        device_be_i[i_byte] ? device_wdata_i[gpo_byte_end - 1 : i_byte * 8] :
                              pcint_mask[gpo_byte_end - 1 : i_byte * 8];
  ...
  assign pcint_status_d = device_be_i[0] ? device_wdata_i[0]     : 1'b0; // special case
  assign pcint_enable_d = device_be_i[2] ? device_wdata_i[16]    : pcint_enable;
  assign pcint_mode_d   = device_be_i[2] ? device_wdata_i[18:17] : pcint_mode;
  assign pcint_i_sel_d  = device_be_i[2] ? device_wdata_i[19]    : pcint_i_sel;
  ...
  assign ctrl_sts_sel   = device_req_i & (reg_addr == GPIO_CTRL_STS_REG[RegAddr-1:0]);
  ...
  always_comb begin
    ...
    else if (pcint_mask_rd_en)
      device_rdata_o = {{(DataWidth - GpoWidth){1'b0}}, pcint_mask};
    else if (ctrl_sts_rd_en)
      device_rdata_o = {12'b0, pcint_i_sel, pcint_mode, pcint_enable, 15'b0, pcint_status};
    else
